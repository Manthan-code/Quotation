  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // Load by quotation ID (edit mode)
        if (mode === "edit" && id) {
...
async function saveQuotation() {
  try {
    ...
    if (!header.clientName || !header.projectId || rows.length === 0) {
  ...
  return (
    ...
    <label className="font-medium" style={FONT}>Project ID</label>
    <input
